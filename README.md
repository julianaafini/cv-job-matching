...
SmartMatch is an AI system designed to streamline the job application process by automatically comparing a candidate's CV with business analytics job descriptions to identify the best-fit positions. This project leverages a combination of semantic similarity using sentence embeddings and skill-level matching via Named Entity Recognition (NER).
